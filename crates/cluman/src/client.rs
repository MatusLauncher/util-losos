...
/// Runs a nerdctl compose file given its path on disk.
///
...
/// (real: `/bin/nerdctl compose -f <path> up -d --remove-orphans`) and return
...
/// requiring nerdctl to be present on the test runner.
...
impl Executor for ProcessExecutor {
    /// Invoke `/bin/nerdctl compose -f <compose_file> up -d --remove-orphans`.
    ///
    ...
    fn run_compose(&self, compose_file: &Path) -> Result<(), String> {
            ...
            .output()
        {
            ...
            Err(e) => Err(format!("failed to spawn `/bin/nerdctl`: {e}")),
        }
    }
}
...
fn execute_compose(task: Task, executor: &(impl Executor + ?Sized)) {
    ...
    match executor.run_compose(&tmp_path) {
        Ok(()) => {
            info!(filename = task.filename, "nerdctl compose up succeeded");
        }
        Err(msg) => {
            error!(
                filename = task.filename,
                error    = %msg,
                "nerdctl compose up failed"
            );
        }
    }
