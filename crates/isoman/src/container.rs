...
//! runs `/bin/podman build`, and extracts the resulting `os.initramfs.tar.gz` artifact.
...
///
/// Steps:
/// 1. `/bin/podman build --build-arg MODE=<mode> --no-cache -t <tag> -f <containerfile> <context>`
/// 2. `/bin/podman create <tag>` → container ID
/// 3. `/bin/podman cp <id>:/os.initramfs.tar.gz <output>`
/// 4. `/bin/podman rm <id>`
pub fn build_initramfs(
    ...
    let build_status = Command::new("/bin/podman")
        .args(args)
        // Inherit stdio so build progress is visible to the user.
        .status()
        .into_diagnostic()
        .wrap_err("/bin/podman not found; install nerdctl")?;

    if !build_status.success() {
        bail!("/bin/podman build failed (exit {})", build_status);
    }
    ...
    let create_out = Command::new("/bin/podman")
        .args(["create", &tag])
        .output()
        .into_diagnostic()
        .wrap_err("/bin/podman create failed")?;

    if !create_out.status.success() {
        bail!(
            "/bin/podman create failed (exit {}): {}",
            create_out.status,
    ...
    let cp_out = Command::new("/bin/podman")
        .args([
            "cp",
        ...
        .output()
        .into_diagnostic()
        .wrap_err("/bin/podman cp failed")?;

    // Always remove the container, even if the copy failed.
    let rm_out = Command::new("/bin/podman")
        .args(["rm", &container_id])
        .output()
        .into_diagnostic()
        .wrap_err("/bin/podman rm failed")?;
    ...
    if !rm_out.status.success() {
        ...
        tracing::warn!(
            ...
            "/bin/podman rm returned non-zero exit code"
        );
    }

    if !cp_out.status.success() {
        bail!(
            "/bin/podman cp failed (exit {}): {}",
            cp_out.status,
