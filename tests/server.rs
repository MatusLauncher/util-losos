...
impl TestServer {
    ...
    /// Run an async ServerState method synchronously in the test thread.
...
fn build_server(state: ServerState) -> App {
    ...
    app.get("/pending", move |_req, res| {
        let st = st.clone();
        async move { res.send_json(&json!({ "pending": st.pending_count().await })) }
    });
...
fn register_client_adds_ip_to_state() {
    ...
    assert!(srv.block_on(srv.state.client_addrs()).contains(&ip));
}
...
fn register_same_client_twice_is_idempotent_in_state() {
    ...
    assert_eq!(srv.block_on(srv.state.client_addrs()).len(), 1);
}
...
fn clients_count_matches_state_client_addrs_len() {
    ...
    assert_eq!(
        client_list(&srv).len(),
        srv.block_on(srv.state.client_addrs()).len()
    );
}
