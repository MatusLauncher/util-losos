...
fn task_available_invokes_executor_with_correct_content() {
    ...
    // The client rewrites compose content to inject GPU devices from the host.
    // Assert that the original service definition is preserved; extra GPU fields
    // (devices:, runtime:, environment:) may be present depending on the host.
    assert!(
        recorded[0].1.contains("image: postgres:16"),
        "executor must receive content preserving the original service definition"
    );
}
...
fn full_client_flow_register_poll_execute() {
    ...
    // 3. Executor must have been called with GPU-enriched compose content.
    // The client rewrites the YAML to inject host GPU devices before execution,
    // so only assert that the original service definition is preserved.
    let recorded = calls.lock().unwrap();
    assert!(
        !recorded.is_empty(),
    ...
    assert!(
        recorded[0].1.contains("image: myapp:2.0"),
        "executor content must preserve the original service definition"
    );
}
        ...
        then.respond_with(move |_: &httpmock::HttpMockRequest| {
            ...
            match next {
                Some(body) => HttpMockResponse::builder()
                    ...
                    .body(body.as_bytes())
                    .build(),
                None => HttpMockResponse::builder().status(204).build(),
            }
        });
    });
